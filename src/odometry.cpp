...
#include "odometry_ros2/odometry.hpp"
  ...
  const std::shared_ptr<sensor_msgs::msg::JointState const> & joint_state)
{
  ...
  //2sungryul add
  //RCLCPP_INFO(nh_->get_logger(), "pos0:%lf,pos1:%lf",joint_state->position[0],joint_state->position[1]);
...
bool Odometry::calculate_odometry(const rclcpp::Duration & duration)
{
  ...
  RCLCPP_INFO(nh_->get_logger(), "x : %f, y : %f, theta:%f", robot_pose_[0], robot_pose_[1], robot_pose_[2]);
